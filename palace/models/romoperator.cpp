...
RomOperator::CalculateNormalizedPROMMatrices(const Units &units) const
{
  ...
  //
  // Lumped ports are real, at the beginning and in order. Wave-port modes follow the
  // lumped ports in the basis (added by AddWavePortModesForSynthesis); they may have
  // both real and imaginary parts. The synthesised matrices are scaled symmetrically by
  // orth_R for those rows as well so the port-block diagonal entries take their
  // physical port-impedance value.
  long n_port_rows = space_op.GetLumpedPortOp().Size();
  // Wave-port basis vectors come right after lumped ports; count how many were actually
  // added (real + imaginary parts each count separately, see AddWavePortModesForSynthesis
  // → UpdatePROM).
  long n_waveport_rows = 0;
  if (!Mwp_p_r.empty())
  {
    for (long j = n_port_rows; j < static_cast<long>(v_node_label.size()); j++)
    {
  ...
  for (long j = 0; j < n_port_rows + n_waveport_rows; j++)
  {
    v_conc[j] = orth_R(j, j);
  }
