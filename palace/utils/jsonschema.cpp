...
// document. Recurses through `properties`, resolves in-document `#/$defs/...`
// refs, and follows `items` for arrays. External-file $ref support was
// removed when the schema collapsed to a single generated file.
    ...
    for (const auto &[_k, v] : props_it->items())
    {
      if (v.contains("$ref"))
      {
        ...
        if (ref_raw.rfind("#/$defs/", 0) == 0)
        {
          ...
          {
            FindAllSchemasByKey(defs[def_name], key, defs, results);
          }
        }
